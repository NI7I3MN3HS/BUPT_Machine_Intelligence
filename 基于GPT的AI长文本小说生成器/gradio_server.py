...
def on_select(instruction1, instruction2, instruction3, evt: gr.SelectData):
    ...
    selected_plan = [instruction1, instruction2,
                     instruction3][selected_plan - 1]
...
with gr.Blocks(
        ...
        with gr.Row():
            with gr.Column():
                ...
                written_paras = gr.Textbox(
                    label="文章内容 (可编辑)", max_lines=21, lines=21)
            with gr.Column():
                with gr.Box():
                    gr.Markdown("### 剧情模式\n")
    ...
    with gr.Tab("自选择剧情"):
        ...
        btn_step.click(
            controled_step,
            inputs=[short_memory, long_memory,
                    selected_instruction, written_paras],
            outputs=[
                short_memory,
                long_memory,
                written_paras,
